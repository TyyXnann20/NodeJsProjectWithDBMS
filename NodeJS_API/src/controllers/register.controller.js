...
const create =  (req,res) => {
    ...
    db.query("SELECT * FROM register WHERE email = ?", [body.email] , (err,rows)=>{
        if(err){
            res.json({
                error:true,
                message : err
            ...
            if(rows.length == 0){
                ...
                var sqlInsert = "INSERT INTO register ( `email`, `password`) VALUES (?,?)"
                db.query(sqlInsert,[body.email, password],(error,rows)=>{
                    if(error){
                        res.json({
                            error : true,
                            message : error
      ...
      (err, rows) => {
        if (err) {
          ...
          if (rows.length == 0) {
            ...
            var password = bcrypt.hashSync(body.password, 10);
            var sqlInsert =
              "INSERT INTO customers ( firstname, lastname, gender, dob,tel, email, password,  is_active) VALUES (?,?,?,?,?,?,?,?)";
            db.query(
              sqlInsert,
              [
                body.firstname,
