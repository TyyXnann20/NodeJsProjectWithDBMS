...
const create = (req,res) => {
    ...
    var sql = "INSERT INTO `category`(`name`, `image`) VALUES (?,?)"
    db.query(sql,[name,image],(error,rows)=>{
        if(error){
            res.json({
                error : true,
                message : error
...
const update = (req,res) => {
    ...
    var sql = "UPDATE `category` SET `name` = ?, `image` = IFNULL(?, `image`) WHERE category_id = ?";
    db.query(sql,[name,image, category_id],(error,rows)=>{
        if(error){
            res.json({
                error : true,
                message : error
