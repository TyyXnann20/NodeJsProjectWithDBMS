...
const CountCustomer = (req, res) => {
  var sql = "SELECT COUNT(customer_id) AS allorder FROM customers";
...
const CountRevenue = (req, res) => {
  var sql = "SELECT sum(total_order) as allsum FROM `order`";
  db.query(sql, (err, result) => {
    if (!err) {
      res.json({
        list: result,
...
const CountOrder = (req, res) => {
  var sql = "SELECT COUNT(order_id) AS od FROM `order`";
  db.query(sql, (err, result) => {
    if (!err) {
      res.json({
        list: result,
...
const CountStockIn = (req, res) => {
  var sql = "SELECT sum(qty) AS quantity FROM  product";
  db.query(sql, (err, result) => {
    if (!err) {
      res.json({
        list: result,
...
const CountStockOut = (req, res) => {
  var sql = "SELECT SUM(qty) as allqty FROM orderdetails";
  db.query(sql, (err, result) => {
    if (!err) {
      res.json({
        list: result,
...
const getByMonth = (req, res) => {
  var sql =
    "SELECT MONTH(o.create_at) AS month, COUNT(DISTINCT o.order_id) AS total_orders, " +
    "SUM(od.qty * p.price) AS total_price FROM `order` o " +
    ...
    (err, rows) => {
      if (err) {
        ...
        if (rows.length == 0) {
          ...
          var sqlInsert =
            "INSERT INTO customers ( firstname, lastname, gender, dob, email, password, image, is_active) VALUES (?,?,?,?,?,?,?,?)";
          db.query(
            sqlInsert,
            [
              body.firstname,
...
const getByDay = (req, res) => {
  var dayby = req.body;
  var sql = "SELECT * FROM order WHERE DATE(create_at) = ?";
  db.query(sql, [dayby], (err, result) => {
    if (err) {
      res.json({
        err: true,
        message: err,
...
const update = (req, res) => {
  ...
  var sqlUpdate =
    "UPDATE customers SET firstname=?, lastname=?, gender=?, dob=?, tel=?, email=?, is_active=?, image=IFNULL(?, image) WHERE customer_id = ?";
  db.query(
    sqlUpdate,
    [
      body.firstname,
